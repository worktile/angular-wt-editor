        ...
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                function set_preview_scroll(editor_scroll) { // 设置预览的滚动位置
                    var lastPosition = 0;
                    var nextPosition = $('.wt-editor-container-preview article').outerHeight() - $('.ui-layout-east').height(); // 这是总共可以scroll的最大幅度
                    if (editor_scroll.lastMarker !== undefined) { // 最开始的位置没有marker
                        lastPosition = $('.wt-editor-container-preview article').find('>[data-line="' + editor_scroll.lastMarker + '"]').get(0).offsetTop;
                    }
                    if (editor_scroll.nextMarker !== undefined) { // 最末尾的位置没有marker
                        nextPosition = $('.wt-editor-container-preview article').find('>[data-line="' + editor_scroll.nextMarker + '"]').get(0).offsetTop;
                ...
                if (editor_theme == undefined) {
                    editor_theme = 'tomorrow_night_blue';
                }
